...
func _physics_process(delta: float) -> void:
	...
	_next_direction = (transform.basis * Vector3(_input_direction.x, 0,_input_direction.y)).normalized().rotated(Vector3.UP, -rotation.y)
	# if any input
	if _next_direction:
	...
	if not _input_direction.is_zero_approx():
		look_at(player_visual_node.global_position + Vector3(_input_direction.x,0,_input_direction.y), Vector3(0, 1, 0))
